...
export default function startWatch() {
  ...
  if (!apiKey) {
    throw new Error(
      "Api key not set in environment variable VIRUSTOTALAPIKEY."
    );
  }
  ...
  if (!timeout) {
    throw new Error(
      "Timeout not set in environment variable VIRUSTOTALTIMEOUT."
    );
  }
  if (timeout < 1) {
  ...
  function onExit(type: string) {
    ...
    client.command(
      ["unsubscribe", dir_of_interest, subscriptionName],
      () => {}
    );
    console.log(type);
    process.exit();
    ...
    (error: Error | null | undefined) => {
      ...
      client.command(
        ...
        (error: Error | null | undefined, resp: any) => {
          ...
          makeTimeConstrainedSubscription(
            client,
            resp.watch,
            resp.relative_path
          );
