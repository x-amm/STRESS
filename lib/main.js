...
if (!dir_of_interest) {
    throw new Error("Directory not set in environment variable DIRECTORYOFINTEREST.");
}
...
function makeTimeConstrainedSubscription(client, watch, relative_path) {
...
function getReport(hash, fileName) {
    const url = "https://www.virustotal.com/vtapi/v2/file/report?apikey=" +
        apiKey +
        "&resource=" +
        hash;
...
function waitForRateLimit(response, callback, options) {
...
function checkStatus(response) {
    if (response.statusCode === 204) {
        console.log("Request rate limit exceeded. You are making more requests than allowed.");
    }
    if (response.statusCode === 400) {
        console.log("Bad request.Your request was somehow incorrect.This can be caused by missing arguments or arguments with wrong values.");
    }
    if (response.statusCode === 403) {
        console.log("Forbidden.You don 't have enough privileges to make the request. You may be doing a request without providing an API key or you may be making a request to a Private API without having the appropriate privileges.");
    }
}
